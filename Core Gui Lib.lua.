--[[
    Smart Tab Library v21
    + Inject into native Roblox tabs (Settings, Configurações, etc.)
    + Divider alias for Label
...
-- Encontra botão nativo do HubBar pelo texto
local function findNativeTabButton(hub, tabName)
    if not hub then return nil end
    local target = string.lower(tostring(tabName or ""))
        ...
        for _, child in ipairs(parent:GetChildren()) do
            if child:IsA("TextButton") and not child:GetAttribute("SmartTab") then
                ...
                for _, t in ipairs(texts) do
                    ...
                    if low == target or string.find(low, target, 1, true) or string.find(target, low, 1, true) then
                        return child, t
                    end
                end
            end
...
local function ensureNativeTab(tabName)
    ...
    local target = string.lower(tostring(tabName))
...
function Divider(data)
    if type(data) ~= "table" then return end
    table.insert(PendingElements, {
        Type = "Divider", Tab = data.Tab, Text = data.Text or "Seção", Icon = data.Icon,
        Description = data.Description, SetOption = data.SetOption
    })
end
...
local function process()
    ...
    for _, el in ipairs(PendingElements) do
        local tabData = getTabData(el.Tab)
        if tabData and tabData.content then
            if el._done then
                ...
                local ok = pcall(function()
                    ...
                    elseif el.Type == "Label" or el.Type == "Divider" then addLabel(tabData, el)
