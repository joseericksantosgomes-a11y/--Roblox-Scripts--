--[[
    Smart Tab Library v14
    - Cada tab com PageView
    - Troca de tab esconde/mostra corretamente
    - Ao reabrir o menu, volta na tab que estava
]]
...
local LastActiveName = nil -- guarda a última tab custom selecionada
local HubScroll = nil
    ...
    for _, child in ipairs(scroll:GetChildren()) do
        if child:IsA("TextButton") and not child:GetAttribute("SmartTab") and not child:GetAttribute("SmartHook") then
            ...
            child.MouseButton1Click:Connect(function()
                deselectAllCustom()
                LastActiveName = nil -- saiu da custom
            end)
        end
    end
end
...
local function process()
    local hub, page, inner, settings = findHubAndPage()
    ...
    -- Restaura a tab que estava aberta
...
-- Detecta quando o menu abre (SettingsShield fica Visible)
task.spawn(function()
