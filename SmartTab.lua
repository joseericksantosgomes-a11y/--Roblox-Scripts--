...
local function clearNativeSelections(hubBarContainer)
    for _, child in ipairs(hubBarContainer:GetDescendants()) do
        if child:IsA("ImageLabel") and child.Name == "TabSelection" then
            local parentBtn = child:FindFirstAncestorWhichIsA("TextButton")
            if parentBtn and not parentBtn:GetAttribute("SmartTab") then
                child.Visible = false
            end
        end
    end
...
local function createTab(hubBarContainer, page, config)
    ...
    if not scroll then
        ...
        scroll.ZIndex = 4
        scroll.ClipsDescendants = true -- IMPORTANTE pra não sair da barra
        ...
        for _, child in ipairs(hubBarContainer:GetChildren()) do
            if child:IsA("TextButton") and child.Name:find("Tab") and not child:GetAttribute("SmartTab") then
                child.Parent = scroll
                -- força tamanho fixo pra não estourar
                if child.Size.X.Scale > 0 then
                    child.Size = UDim2.new(0, math.clamp(child.AbsoluteSize.X, 80, 160), 1, 0)
                end
            end
        end
    ...
    myTab.ZIndex = 5
    myTab.Text = ""
    myTab.Size = UDim2.new(0, 120, 1, 0)
    ...
    Instance.new("UITextSizeConstraint", title).MaxTextSize = 13
    ...
    for _, child in ipairs(scroll:GetChildren()) do
        if child:IsA("TextButton") and not child:GetAttribute("SmartTab") then
            child.MouseButton1Click:Connect(function()
                clearCustomSelections(scroll)
                ActiveTab = nil
                if myPageView then myPageView.Visible = false end
            end)
        end
    end
...
local function applyPosition(frame, position)
    ...
    if position == "Left" then
        frame.Size = UDim2.new(0.5, -6, frame.Size.Y.Scale, frame.Size.Y.Offset)
        frame.LayoutOrder = frame.LayoutOrder or 0
    elseif position == "Right" then
        frame.Size = UDim2.new(0.5, -6, frame.Size.Y.Scale, frame.Size.Y.Offset)
        frame.LayoutOrder = (frame.LayoutOrder or 0) + 1
    end
    -- None = full width (já é o padrão)
end
