--[[
    Smart Tab Library v11
    Mais estável - funciona mesmo com menu fechado
]]
...
local function waitForHubBar()
    local ok, result = pcall(function()
        local robloxGui = CoreGui:FindFirstChild("RobloxGui") or CoreGui:WaitForChild("RobloxGui", 10)
        if not robloxGui then return nil end

        local settingsShield = robloxGui:FindFirstChild("SettingsClippingShield")
        if not settingsShield then
            settingsShield = robloxGui:WaitForChild("SettingsClippingShield", 10)
        end
        ...
        local hubBarContainer = tabHeader:FindFirstChild("HubBarContainer")
            or tabHeader:WaitForChild("HubBarContainer", 5)

        return hubBarContainer, page
    end)
...
local function createTab(hubBarContainer, page, config)
    ...
    if not scroll then
        ...
        if originalList then
            pcall(function() originalList:Destroy() end)
        end
    end
    HubScroll = scroll

    local tabName = "SmartTab_" .. tostring(config.Name or "Custom"):gsub("%s+", "")
    ...
    local isAssetId = string.find(iconStr, "rbxassetid://") or tonumber(iconStr)
    ...
    title.Text = tostring(config.Name or "Tab")
...
local function createDropdown(parent, data)
    ...
    label.Size = UDim2.new(0.4, -20, 0, 50)
    ...
    dropBtn.Position = UDim2.new(1, 0, 0, 25)
    ...
    dropBtn.MouseButton1Click:Connect(function()
        if isOpen then
            ...
            local h = #options * 36
            TweenService:Create(listFrame, TweenInfo.new(0.2), {Size = UDim2.new(0.6, 0, 0, h)}):Play()
...
local function createSelector(parent, data)
    ...
    leftBtn.Size = UDim2.new(0, 32, 1, 0)
    ...
    rightBtn.Position = UDim2.new(1, 0, 0, 0)
    rightBtn.Size = UDim2.new(0, 32, 1, 0)
...
local function processEverything()
    ...
    if hub then
        for _, cfg in ipairs(PendingTabs) do
            pcall(createTab, hub, page, cfg)
        end
        clearTable(PendingTabs)
...
task.spawn(function()
    for i = 1, 8 do
        task.wait(1.5)
        if #PendingTabs > 0 or next(CreatedTabs) == nil then
            processEverything()
        end
        if next(CreatedTabs) then break end
    end
end)

CoreGui.DescendantAdded:Connect(function(desc)
    if desc.Name == "HubBarContainer" or desc.Name == "TabHeaderContainer" then
        task.wait(0.8)
