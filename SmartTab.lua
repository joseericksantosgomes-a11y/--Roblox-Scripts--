...
local function createTab(hub, page, inner, config)
    ...
    -- Botão já existe mas Tabs perdeu a referência → reassocia
...
local function process()
    ...
    local leftTabs = {}
    for _, cfg in ipairs(PendingTabs) do
        local ok = pcall(createTab, hub, page, inner, cfg)
    ...
    -- Elements (custom Tab ou PutOn nativo)
    local leftEls = {}
    for _, el in ipairs(PendingElements) do
        if el._done then
            -- skip
        else
            local targetName = el.Tab
            if type(el.PutOn) == "string" and el.PutOn ~= "" then
                targetName = el.PutOn
            end
            if not targetName then
                ...
                local tabData = nil
                local okGet, result = pcall(getTabData, targetName)
                if okGet then tabData = result end
...
task.spawn(function()
    ...
    while true do
        ...
        elseif visible then
            -- menu já aberto: tenta processar pendentes
