...
local function updateTabScrollCanvas(scroll)
    ...
    -- Canvas só cresce conforme o conteúdo real das tabs
    local w = layout.AbsoluteContentSize.X
    if w < 1 then
        -- fallback: soma largura dos filhos
        w = 0
        for _, child in ipairs(scroll:GetChildren()) do
            if child:IsA("GuiObject") and child.Visible then
                w = w + child.AbsoluteSize.X
            end
        end
    ...
    scroll.CanvasSize = UDim2.new(0, math.max(w + 12, 0), 0, 0)
    scroll.ScrollingEnabled = w > scroll.AbsoluteSize.X + 1
end
...
local function ensureScroll(hub)
    ...
    scroll.AutomaticCanvasSize = Enum.AutomaticSize.None -- controlamos manualmente
...
local function createTab(hub, page, inner, config)
    ...
    btn.LayoutOrder = 500 + customCount
    btn.ZIndex = 5
    btn.Parent = scroll
    task.defer(updateTabScrollCanvas, scroll)
