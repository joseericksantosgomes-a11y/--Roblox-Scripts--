...
local function createTab(hubBarContainer, config)
    ...
    for _, child in ipairs(hubBarContainer:GetChildren()) do
        if child:IsA("TextButton") and child.Name:find("Tab") and not child:GetAttribute("SmartTab") then
            ...
            if child.Size.X.Scale > 0 then
                child.Size = UDim2.new(0, 155, 1, 0)
            end
        end
    end

    local oldList = hubBarContainer:FindFirstChildOfClass("UIListLayout")
    if oldList and oldList.Parent == hubBarContainer then
        oldList:Destroy()
    end

    local tabName = "SmartTab_" .. (config.Name or "Custom")
    ...
    myTab.ZIndex = 3
    myTab.Text = ""
    myTab.Size = UDim2.new(0, 155, 1, 0)
    ...
    tabSelection.Position = UDim2.new(0, 3, 1, -2)
    tabSelection.Size = UDim2.new(1, -6, 0, 2)
    ...
    myTab.MouseButton1Click:Connect(function()
        ...
        if config.Content and config.Content ~= "default" and config.Content ~= "" then
            ...
            if customFrame and (customFrame:IsA("Frame") or customFrame:IsA("ScrollingFrame") or customFrame:IsA("CanvasGroup")) then
                ...
                if contentContainer then
                    customFrame.Parent = contentContainer
                end
            end
        end
    end)
