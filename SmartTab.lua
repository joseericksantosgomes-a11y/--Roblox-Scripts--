--[[
    Smart Tab Library v17
    + Slider, Input
    + SetOption DENTRO do Callback:
        Callback = function(ctrl)  -- ou (value, ctrl)
            ctrl.SetOption("Locked")
            ctrl.SetOption("Unlocked")
        end
]]
...
local function makeCtrl(frame, stateRef)
    return {
...
local function addToggle(tabData, data)
    ...
    local ctrl = makeCtrl(frame, stateRef)
...
local function addInput(tabData, data)
    ...
    -- quando Locked via ctrl, também bloqueia edição
    local oldSet = ctrl.SetOption
    ctrl.SetOption = function(opt)
        oldSet(opt)
        if type(opt) == "string" and opt:lower() == "locked" then
            box.TextEditable = false
        elseif type(opt) == "string" and opt:lower() == "unlocked" then
